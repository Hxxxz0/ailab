...
def _median_filter_1d(x, k):
    """Sliding median along axis 0 (edge-replicated). x: [T, D], k: odd kernel."""
    if k <= 1 or x.shape[0] < k:
        return x
    r = k // 2
    xp = np.pad(x, ((r, r), (0, 0)), mode="edge")
    win = np.stack([xp[i : i + x.shape[0]] for i in range(k)], axis=0)  # [k, T, D]
    return np.median(win, axis=0)


def _gaussian_filter_1d(x, sigma):
    """Gaussian smoothing along axis 0 (edge-replicated). x: [T, D]."""
    if sigma <= 0:
        return x
    ...
    for i, wi in enumerate(w):
        out += wi * xp[i : i + x.shape[0]]
    return out.astype(x.dtype)


def temporal_smooth_output(wrist_pos, wrist_aa, dof_pos, wrist_med=5, wrist_sigma=2.0, dof_med=3, dof_sigma=1.0):
    """Post-hoc temporal filter to remove isolated per-frame IK artifacts (wrist
    teleports, finger pops).  The MANO source trajectories are smooth, so median
    (kills outliers) + gaussian (denoises) toward temporal neighbours REDUCES
    error rather than hurting fidelity.  Rotations are smoothed as quaternions
    with hemisphere alignment to avoid sign-flip discontinuities."""
    from main.dataset.transform import aa_to_quat, quat_to_rotmat, rotmat_to_aa

    wrist_pos = np.asarray(wrist_pos, dtype=np.float32)
    dof_pos = np.asarray(dof_pos, dtype=np.float32)

    wp = _gaussian_filter_1d(_median_filter_1d(wrist_pos, wrist_med), wrist_sigma)
    dp = _gaussian_filter_1d(_median_filter_1d(dof_pos, dof_med), dof_sigma)

    # Rotation: aa -> quat (wxyz), hemisphere-align, median+gaussian, renormalize -> aa
    q = aa_to_quat(torch.as_tensor(wrist_aa, dtype=torch.float32)).cpu().numpy()  # [T,4] wxyz
    for i in range(1, q.shape[0]):
        if float(np.dot(q[i], q[i - 1])) < 0:
            q[i] = -q[i]
    q = _gaussian_filter_1d(_median_filter_1d(q, wrist_med), wrist_sigma)
    q /= np.clip(np.linalg.norm(q, axis=-1, keepdims=True), 1e-8, None)
    aa = rotmat_to_aa(quat_to_rotmat(torch.as_tensor(q, dtype=torch.float32))).cpu().numpy()
    return wp.astype(np.float32), aa.astype(np.float32), dp.astype(np.float32)
...
class Mano2Dexhand:
    def __init__(self, args, dexhand, obj_urdf_path):
        ...
        with open(os.path.join(asset_root, asset_file), "rb") as f:
        ...
        elif self.dexhand.name == "sharpa":
            # Sharpa's PIP/DIP/IP flexion joints have limits [0, +], so a plain
            # zero init sits exactly on the lower bound. The IK optimizer then gets
            # clamped there (clamp has zero grad past the boundary) and these joints
            # never leave 0 -> the fingers only bend at the knuckle and stay straight
            # past it. Start the flexion joints slightly bent, off the boundary, so IK
            # can distribute finger curl into them.
            lower = self.dexhand_dof_lower_limits.cpu().numpy()
            upper = self.dexhand_dof_upper_limits.cpu().numpy()
            isaac_dof_names = [
                self.gym.get_asset_dof_name(dexhand_asset, i) for i in range(self.num_dexhand_dofs)
            ]
            init = np.zeros(self.num_dexhand_dofs, dtype=np.float32)
            for i, nm in enumerate(isaac_dof_names):
                if nm.endswith("PIP") or nm.endswith("DIP") or nm.endswith("_IP"):
                    init[i] = 0.3
                elif nm.endswith("MCP_FE") or nm.endswith("CMC_FE"):
                    init[i] = 0.2
            default_dof_state["pos"] = np.clip(init, lower, upper)
        else:
            default_dof_state["pos"] = np.clip(
                np.zeros(self.num_dexhand_dofs, dtype=np.float32),
                self.dexhand_dof_lower_limits.cpu().numpy(),
                self.dexhand_dof_upper_limits.cpu().numpy(),
            )
    ...
    def fitting(self, max_iter, obj_trajectory, target_wrist_pos, target_wrist_rot, target_mano_joints):
        ...
        while (self.headless and iter < max_iter) or (
            ...
            tracking_loss = torch.mean(torch.norm(pk_joints - target_joints, dim=-1) * weight[None])
            smooth_loss = torch.zeros((), device=self.sim_device)
            acc_loss = torch.zeros((), device=self.sim_device)
            if self.args.dof_smooth_weight > 0 and opt_dof_pos_clamped.shape[0] > 1:
                dof_vel = opt_dof_pos_clamped[1:] - opt_dof_pos_clamped[:-1]
                smooth_loss = torch.mean(torch.sum(dof_vel * dof_vel, dim=-1))
            if self.args.dof_acc_weight > 0 and opt_dof_pos_clamped.shape[0] > 2:
                dof_acc = opt_dof_pos_clamped[2:] - 2 * opt_dof_pos_clamped[1:-1] + opt_dof_pos_clamped[:-2]
                acc_loss = torch.mean(torch.sum(dof_acc * dof_acc, dim=-1))
            # The frames are optimized independently (num_envs == T frames in order), so
            # without a temporal prior the wrist pose can jump between IK solutions on
            # adjacent frames -> teleporting/jitter. Penalize wrist acceleration (2nd
            # difference over frames): kills spikes while leaving smooth motion untouched.
            wrist_smooth_loss = torch.zeros((), device=self.sim_device)
            wrist_smooth_weight = getattr(self.args, "wrist_smooth_weight", 0.0)
            if wrist_smooth_weight > 0 and opt_wrist_pos.shape[0] > 2:
                wpos_acc = opt_wrist_pos[2:] - 2 * opt_wrist_pos[1:-1] + opt_wrist_pos[:-2]
                wrot_acc = opt_wrist_rot[2:] - 2 * opt_wrist_rot[1:-1] + opt_wrist_rot[:-2]
                wrist_smooth_loss = torch.mean(torch.sum(wpos_acc * wpos_acc, dim=-1)) + torch.mean(
                    torch.sum(wrot_acc * wrot_acc, dim=-1)
                )
            loss = (
                tracking_loss
                + self.args.dof_smooth_weight * smooth_loss
                + self.args.dof_acc_weight * acc_loss
                + wrist_smooth_weight * wrist_smooth_loss
            )
            opti.zero_grad()
            loss.backward()
            opti.step()
        ...
        out_wrist_pos = opt_wrist_pos.detach().cpu().numpy()
        out_wrist_aa = rot6d_to_aa(opt_wrist_rot).detach().cpu().numpy()
        out_dof_pos = opt_dof_pos_clamped.detach().cpu().numpy()

        if getattr(self.args, "post_smooth", 1) and out_wrist_pos.shape[0] > 5:
            out_wrist_pos, out_wrist_aa, out_dof_pos = temporal_smooth_output(
                out_wrist_pos, out_wrist_aa, out_dof_pos,
                wrist_med=getattr(self.args, "post_wrist_med", 5),
                wrist_sigma=getattr(self.args, "post_wrist_sigma", 2.0),
                dof_med=getattr(self.args, "post_dof_med", 3),
                dof_sigma=getattr(self.args, "post_dof_sigma", 1.0),
            )
            out_dof_pos = np.clip(
                out_dof_pos,
                self.dexhand_dof_lower_limits.cpu().numpy(),
            ...
            cprint("[post_smooth] applied temporal wrist+dof filter", "cyan")

        to_dump = {
            "opt_wrist_pos": out_wrist_pos,
            "opt_wrist_rot": out_wrist_aa,
