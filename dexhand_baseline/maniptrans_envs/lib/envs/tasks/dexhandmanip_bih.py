...
class DexHandManipBiHEnv(VecTask):
    ...
    def __init__(
        ...
        *,
        ...
        assert not use_quat_rot, "ResDexHand direct-action policy currently supports useQuatRot=false only."
        assert (
            self.cfg["env"]["bimanual_mode"] == "united"
        ), "ResDexHandBiH direct-action policy currently supports bimanual_mode=united only."
        ...
        self.cfg["env"]["numActions"] = single_hand_actions * (
    ...
    def pre_physics_step(self, actions):
        ...
        rh_offset = 0
        lh_offset = root_control_dim + self.num_dexhand_rh_dofs

        rh_dof_pos = actions[
            :, rh_offset + root_control_dim : rh_offset + root_control_dim + self.num_dexhand_rh_dofs
        ]
        rh_dof_pos = torch.clamp(rh_dof_pos, -1, 1)

        lh_dof_pos = actions[
            :, lh_offset + root_control_dim : lh_offset + root_control_dim + self.num_dexhand_lh_dofs
        ]
        ...
        if self.use_pid_control:
            rh_position_error = actions[:, rh_offset + 0 : rh_offset + 3]
            ...
            lh_position_error = actions[:, lh_offset + 0 : lh_offset + 3]
            ...
            rh_rotation_error = actions[:, rh_offset + 3 : rh_offset + root_control_dim]
            ...
            lh_rotation_error = actions[:, lh_offset + 3 : lh_offset + root_control_dim]
            ...
            rh_force = actions[:, rh_offset + 0 : rh_offset + 3] * self.dt * self.translation_scale * 500
            rh_torque = actions[:, rh_offset + 3 : rh_offset + 6] * self.dt * self.orientation_scale * 200
            lh_force = actions[:, lh_offset + 0 : lh_offset + 3] * self.dt * self.translation_scale * 500
            lh_torque = actions[:, lh_offset + 3 : lh_offset + 6] * self.dt * self.orientation_scale * 200
...
def compute_imitation_reward(
    ...
    else:  # * for evaluation, use the same standard as in the paper
        failed_execute = (
            (
