...
echo "=== Running SAM3 video segmentation (objects, headless text prompt) ==="
# HEADLESS (no display needed): segment the object with a SAM3 *text* prompt by default.
# The original --click GUI (needs SAM3_DISPLAY / VNC) is not used. For more precise selection
# without a display, set SAM3_OBJ_POINTS="x,y" to a positive pixel on the object in the ref
# frame (extract it once: ffmpeg -i VIDEO -vf select=eq(n\,N) -vframes 1 frame.png, eyeball a pixel).
for OBJ_NAME in "${OBJECT_NAMES[@]}"; do
    OBJ_ID="${OBJ_NAME// /_}"
    if [ -n "${SAM3_OBJ_POINTS:-}" ]; then
        HF_HUB_OFFLINE=1 python run_sam3_video.py \
            --video "$VIDEO_PATH" \
            --points "$SAM3_OBJ_POINTS" --point_labels "${SAM3_OBJ_POINT_LABELS:-1}" \
            --obj_id "$OBJ_ID" \
            --frame_idx "$n"
    else
        HF_HUB_OFFLINE=1 python run_sam3_video.py \
            --video "$VIDEO_PATH" \
            --text "${SAM3_OBJ_TEXT:-$OBJ_NAME}" \
            --obj_id "$OBJ_ID" \
            --frame_idx "$n"
    fi
done
